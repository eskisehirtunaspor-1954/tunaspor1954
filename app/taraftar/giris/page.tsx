...
export default function TaraftarGirisPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError(null);
    const res = await fetch("/api/taraftar/auth/login", {
      method: "POST",
    ...
    router.push("/taraftar/panel");
  }

  return (
    <div className="max-w-md mx-auto px-4 py-24">
      <p className="eyebrow mb-3 text-center">Tunaspor 1954</p>
      <h1 className="font-display text-3xl mb-8 text-center">Taraftar Girişi</h1>

      <form onSubmit={handleSubmit} className="glass-panel p-6 space-y-4">
        <label className="block">
          <span className="text-sm text-tuna-mist block mb-1">E-posta veya Telefon</span>
          <input
            required
            value={identifier}
        ...
        <p className="text-xs text-tuna-mist text-center">
          Henüz üye değil misin? <Link href="/taraftar/kayit" className="text-tuna-gold hover:underline">Hemen üye ol</Link>
        </p>
      </form>
    </div>
  );
