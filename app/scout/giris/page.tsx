...
export default function ScoutGirisPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError(null);
    const res = await fetch("/api/scout/auth/login", {
      method: "POST",
    ...
    router.push("/scout/panel");
  }

  return (
    <div className="max-w-md mx-auto px-4 py-24">
      <p className="eyebrow mb-3 text-center">Tunaspor 1954</p>
      <h1 className="font-display text-3xl mb-8 text-center">Scout Girişi</h1>
      ...
      <form onSubmit={handleSubmit} className="glass-panel p-6 space-y-4">
        ...
        <p className="text-xs text-tuna-mist text-center">
          Hesabın yok mu? <a href="/scout/basvuru" className="text-tuna-gold underline">Başvuru yap</a>
        </p>
      </form>
    </div>
  );
